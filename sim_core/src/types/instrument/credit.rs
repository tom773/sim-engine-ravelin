...
impl CreditRegistry {
    ...
    pub fn draw_from_facility(
        &mut self,
        facility_id: Uuid,
        loan: Loan,
        amount: Money,
    ) -> Result<(), String> {
        let facility = self.facilities.get_mut(&facility_id)
            .ok_or_else(|| format!("Facility {} not found", facility_id))?;
        ...
        self.loans_by_lender
            ...
            .map(|ids| {
                ids.iter()
                    ...
                    .map(|loan| loan.details.outstanding_principal)
                    .sum()
            })
...
impl LoanServicer {
    pub fn new(current_date: NaiveDate) -> Self {
        Self {
            current_date,
            rate_environment: RateEnvironment::default(),
        }
    }
    ...
    fn calculate_annuity_payment(&self, loan: &LoanDetails) -> Money {
        let rate = self.calculate_rate(loan);
        let period_rate = rate * Rate::from_f64(self.payment_frequency_to_year_frac(&loan.payment_frequency)).unwrap_or(Rate::ZERO);
        let n = self.count_payment_periods(loan) as i64;
        
        if period_rate == Rate::ZERO {
            loan.outstanding_principal / n as f64
        } else {
            let factor = period_rate * (Rate::ONE + period_rate).powi(n) / ((Rate::ONE + period_rate).powi(n) - Rate::ONE);
