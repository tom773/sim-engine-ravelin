...
fn classify_market(bond_type: BondType, issue: NaiveDate, maturity: NaiveDate) -> InstrumentMarket {
    ...
    match (bond_type, money) {
        (BondType::Government, true) | (BondType::InterbankLoan, true) => InstrumentMarket::MoneyMarket(MoneyMarketSegment::SovereignShortTerm),
        (BondType::Corporate,  true) => InstrumentMarket::MoneyMarket(MoneyMarketSegment::CorporateShortTerm),
        (BondType::Government, false) | (BondType::InterbankLoan, false) => InstrumentMarket::CapitalMarket(CapitalMarketSegment::SovereignLongTerm),
...
impl CashBuilder {
    pub fn market(mut self, m: InstrumentMarket) -> Self { self.market = Some(m); self }
    pub fn build(self) -> Instrument {
        Instrument {
            id: self.id,
            instrument_type: InstrumentType::Cash(self.details),
            instrument_market: self.market.unwrap_or(
                InstrumentMarket::MoneyMarket(MoneyMarketSegment::Interbank)
            ),
...
impl BondBuilder {
    ...
    pub fn fixed(mut self) -> Self { self.details.cash_flow = CashFlow::Fixed; self }
    pub fn floating(mut self) -> Self { self.details.cash_flow = CashFlow::Floating; self }

    pub fn coupon_bps(mut self, bps: BasisPoints) -> Self { self.details.coupon_rate_bps = bps; self }
    pub fn frequency(mut self, per_year: u32) -> Self { self.details.frequency = per_year; self }
    pub fn day_count(mut self, dc: DayCount) -> Self { self.details.day_count = dc; self }
    pub fn rating(mut self, r: CreditRating) -> Self { self.details.rating = r; self }
    pub fn market(mut self, m: InstrumentMarket) -> Self { self.market = Some(m); self }
...
impl Instrument {
    ...
    pub fn bond_fixed(
        ...
        Self::bond_full(
            id, issuer, bond_type, face_value, issue_date, maturity_date,
            BondTerms::Fixed { coupon_rate_bps_bps, frequency, day_count },
            rating,
            market,
        )
    ...
    pub fn gov_bond(
        ...
        let issue = today();
        let years = years_from_f64(tenor_years)
            .ok_or_else(|| BuildError::BadDates(issue, issue))?;

        let maturity = add_years(issue, years)
            .ok_or_else(|| BuildError::BadDates(issue, issue))?;
    ...
    pub fn gov_bill_months(gov: Government, months: NonZeroU32) -> Result<Self, BuildError> {
        let issue = today();
        let maturity = issue
            .checked_add_months(Months::new(months.get()))
            .ok_or_else(|| BuildError::BadDates(issue, issue))?;
...
fn years_from_f64(t: f64) -> Option<NonZeroU32> {
    if !t.is_finite() || t < 1.0 { return None; }
    let r = t.round();
    if (t - r).abs() > 1e-9 { return None; }
