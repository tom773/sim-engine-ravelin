...
pub enum FinancialEffect {
    ...
    AccrueInterest { instrument_id: InstrumentId, accrued_amount: f64, accrual_date: NaiveDate },
...
impl StateEffectApplicator {
        ...
        if amount_from_cash > 1e-6 {
            if let Some(id) = cash_id {
                ...
                if new_cash_principal < 1e-6 {
                    ...
                    state
                        .financial_system
                        .update_instrument(&id, new_cash_principal)
                        .map_err(EffectError::FinancialSystemError)?;
                }
                ...
                state
                    .financial_system
                    .create_or_consolidate_instrument(recipient_cash)
                    .map_err(EffectError::FinancialSystemError)?;
            }
        }
        ...
        if amount_from_deposits > 1e-6 {
                ...
                from_bs.assets.iter().find(|(_, inst)| inst.details.as_any().is::<DemandDepositDetails>())
            {
                ...
                if new_deposit_principal < 1e-6 {
                    state.financial_system.remove_instrument(dep_id).map_err(EffectError::FinancialSystemError)?;
                } else {
                    state
                        .financial_system
                        .update_instrument(dep_id, new_deposit_principal)
                        .map_err(EffectError::FinancialSystemError)?;
                }

                if let Some(payer_bank_bs) = state.financial_system.balance_sheets.get(&payer_bank_id).cloned() {
                    if let Some((res_id, res_inst)) =
                        payer_bank_bs.assets.iter().find(|(_, i)| i.details.as_any().is::<CentralBankReservesDetails>())
                    {
                        let new_reserves = res_inst.principal - amount_from_deposits;
                        if new_reserves < 1e-6 {
                            state
                                .financial_system
                                .remove_instrument(res_id)
                                .map_err(EffectError::FinancialSystemError)?;
                        } else {
                            state
                                .financial_system
                                .update_instrument(res_id, new_reserves)
                                .map_err(EffectError::FinancialSystemError)?;
                        }
                    }
                }
                ...
                let recipient_bank_id = if state.agents.banks.contains_key(&to) {
                    ...
                    f.bank_id
                } else if to == state.financial_system.government.id {
                    to
                } else if to == state.financial_system.central_bank.id {
                    to
                } else {
                    return Err(EffectError::InvalidState(format!("Could not determine bank for recipient {}", to)));
                };

                let recipient_reserves = reserves!(
                    recipient_bank_id,
                    cb_id,
                    amount_from_deposits,
                    state.current_date,
                    state.financial_system.central_bank.policy_rate_bps + 15.0
                );
                state
                    .financial_system
                    .create_or_consolidate_instrument(recipient_reserves)
                    .map_err(EffectError::FinancialSystemError)?;

                if !state.agents.banks.contains_key(&to)
                    && to != state.financial_system.government.id
                    && to != state.financial_system.central_bank.id
                {
                    let bank_spread_bps =
                        state.agents.banks.get(&recipient_bank_id).map(|b| b.deposit_spread_bps).unwrap_or(-50.0);
                    ...
                    let recipient_deposit =
                        deposit!(to, recipient_bank_id, amount_from_deposits, deposit_rate_bps, state.current_date);
                    state
                        .financial_system
                        .create_or_consolidate_instrument(recipient_deposit)
                        .map_err(EffectError::FinancialSystemError)?;
                }
            }
        }
    ...
    fn apply_deposit_funds(
        state: &mut SimState, depositor: AgentId, bank: AgentId, amount: f64,
    ) -> Result<(), EffectError> {
        ...
        let bank_spread_bps = state.agents.banks.get(&bank).map(|b| b.deposit_spread_bps).unwrap_or(-50.0);
        let policy_rate_bps = state.financial_system.central_bank.policy_rate_bps;
        let deposit_rate_bps = (policy_rate_bps + bank_spread_bps).max(0.0);

        let deposit = deposit!(depositor, bank, amount, deposit_rate_bps, state.current_date);
        state.financial_system.create_or_consolidate_instrument(deposit).map_err(EffectError::FinancialSystemError)?;

        if let Some(depositor_bs) = state.financial_system.balance_sheets.get(&depositor).cloned() {
            if let Some((cash_id, cash_inst)) =
                depositor_bs.assets.iter().find(|(_, i)| i.details.as_any().is::<CashDetails>())
            {
                let new_cash = cash_inst.principal - amount;
                if new_cash < 1e-6 {
                    state.financial_system.remove_instrument(cash_id).map_err(EffectError::FinancialSystemError)?;
                } else {
                    state
                        .financial_system
                        .update_instrument(cash_id, new_cash)
                        .map_err(EffectError::FinancialSystemError)?;
                }
            }
        }
        ...
        state.financial_system.create_or_consolidate_instrument(reserves).map_err(EffectError::FinancialSystemError)?;

        Ok(())
    }

    fn apply_withdraw_funds(
        state: &mut SimState, account_holder: AgentId, bank: AgentId, amount: f64,
    ) -> Result<(), EffectError> {
        ...
        if deposits_at_bank < amount {
            return Err(EffectError::TransactionFailure(
                ...
                format!(
                    ...
                    account_holder, deposits_at_bank, amount
                ),
            ));
        }
        ...
        if let Some(account_holder_bs) = state.financial_system.balance_sheets.get(&account_holder).cloned() {
            if let Some((deposit_id, deposit)) = account_holder_bs
                .assets
                .iter()
                .find(|(_, inst)| inst.debtor == bank && inst.details.as_any().is::<DemandDepositDetails>())
            {
                ...
                if new_principal < 1e-6 {
                    state.financial_system.remove_instrument(deposit_id).map_err(EffectError::FinancialSystemError)?;
                } else {
                    state
                        .financial_system
                        .update_instrument(deposit_id, new_principal)
                        .map_err(EffectError::FinancialSystemError)?;
                }
                ...
                if let Some(bank_bs) = state.financial_system.balance_sheets.get(&bank).cloned() {
                    if let Some((res_id, res_inst)) =
                        bank_bs.assets.iter().find(|(_, i)| i.details.as_any().is::<CentralBankReservesDetails>())
                    {
    ...
    fn apply_inject_liquidity(
        state: &mut SimState, recipients: &[AgentId], amount_per_recipient: f64,
    ) -> Result<(), EffectError> {
        let cb_id = state.financial_system.central_bank.id;

        for recipient in recipients {
            let cash = cash!(*recipient, amount_per_recipient, cb_id, state.current_date);
            state.financial_system.create_or_consolidate_instrument(cash).map_err(EffectError::FinancialSystemError)?;
        }

        Ok(())
    }

    fn apply_accrue_interest(
        state: &mut SimState, instrument_id: InstrumentId, accrued_amount: f64, accrual_date: chrono::NaiveDate,
    ) -> Result<(), EffectError> {
